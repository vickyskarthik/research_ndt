...
# ----- Run each mode -----
def run_mode(name, use_pw, use_imu, radar_path, gt_path, save_path, bag_name, grid1, grid2, grid3):
    ...
    for date in os.listdir(dataset_root):
        if len(date) == 36:  # Only valid sequence folders
            ...
            for m in modes_to_run:
                for (g1, g2, g3) in grid_configs:
                    grid_tag = f"grid_{g1}_{g2}_{g3}"
                    save_path = os.path.join(results_base_dir, grid_tag)
                    os.makedirs(save_path, exist_ok=True)
                    print(f"\n--- Running {m['name']} with grid sizes ({g1}, {g2}, {g3}) ---\n")
