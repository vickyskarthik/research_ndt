...
bool eval (string result_sha,Mail* mail) {
  ...
  if (fs::exists(result_dir) && fs::is_directory(result_dir)) {
        std::cout << "Directory exists. Clearing contents...\n";

        // Iterate through the directory and remove each entry
        for (const auto& entry : fs::directory_iterator(result_dir)) {
            fs::remove_all(entry.path());
        }

        std::cout << "Directory contents cleared.\n";
    } else {
  ...
  for (int32_t i=0; i<28; i++) { //28
    ...
    std::vector<Matrix> poses_gt = loadGtPoses(gt_path);
    std::vector<Matrix> poses_result = loadPoses(result_path);
    // plot status
...
int32_t main (int32_t argc,char *argv[]) {
    ...
    const char* gt_env = std::getenv("GT_PATH");
    const char* res_env = std::getenv("RESULT_PATH");

    if (!gt_env || !res_env) {
        std::cerr << "Missing environment variables GT_PATH or RESULT_PATH" << std::endl;
        return 1;
    }
    gt_path = gt_env;
    result_path = res_env;
